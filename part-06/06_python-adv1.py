""" Project: Music Playlist Organizer"""
import os
import shutil
from mutagen import File
import json

def scan_directory(directory, extensions = ('.mp3', '.wav', '.ogg')):
    music_files = []
...
def save_summary_to_json(music_files, output_files):
    summary = []
    for file in music_files:
        metadata = extract_metadata(file)
        if metadata:
    ...
    with open(output_files, "w") as json_file:
        json.dump(summary, json_file, indent= 4)
    print(f"Summary saved to {output_files}")

def extract_metadata(file_path):
    try:
        audio = File(file_path, easy = True)
        return {
            "title": audio.get("title", ["Unkown Title"])[0],
            "artist": audio.get("artist", ["Unkown Artist"])[0],
            "album": audio.get("album", ["Unkown Album"])[0],
            "genre": audio.get("genre", ["Unkown Genre"])[0],
        }
...
def main():
    ...
    print("Music organization completed!")
